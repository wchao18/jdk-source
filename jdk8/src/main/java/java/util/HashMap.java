...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * The default initial capacity - MUST be a power of two.必须是二的n次幂
     */
    static final int DEFAULT_INITIAL_CAPACITY = 1 << 4; // aka 16，默认容量：左位移4位，即2的4次幂
     ...
     */
    static final float DEFAULT_LOAD_FACTOR = 0.75f;//负载因子：扩容使用，统计学计算出的最合理的值
     ...
     * shrinkage.
     */
    static final int TREEIFY_THRESHOLD = 8;//当链表的值>8，转红黑树(1.8新增)，注意：还要判断table.length是否达到64，如果没有达到64，先扩容
     ...
     */
    static final int MIN_TREEIFY_CAPACITY = 64;//大于64，链表才能进行树化
        ...
        public final boolean equals(Object o) {
            ...
            if (o instanceof Map.Entry) {
                ...
                if (Objects.equals(key, e.getKey()) &&
                    Objects.equals(value, e.getValue()))//key、value都相等Entry才相等
                    return true;
            }
            return false;
        }
     ...
     */
    static final int hash(Object key) {//干扰（扰动）函数；通过key计算hash值，仅仅是hash值，不是存储位置，目的：让hash更散列
        int h;
        return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);//hash后对右16（空位补0）后进行异或，key为null，表示HashMap是支持Key为空的，
    }
    ...
    public HashMap(int initialCapacity, float loadFactor) {//指定初始容量和负载因子
        if (initialCapacity < 0)//初始化容量判断
            throw new IllegalArgumentException("Illegal initial capacity: " +
                                               initialCapacity);
        if (initialCapacity > MAXIMUM_CAPACITY)//是否大于最大容量
            initialCapacity = MAXIMUM_CAPACITY;//最大容量赋值
        if (loadFactor <= 0 || Float.isNaN(loadFactor))//负载因子是否小于零 ||  是一个负数
            throw new IllegalArgumentException("Illegal load factor: " +
                                               loadFactor);
        this.loadFactor = loadFactor;
        this.threshold = tableSizeFor(initialCapacity);//扩容临界点
    }
     ...
     */
    public HashMap() {//没有new对象？？？构造造一个空的 HashMap，默认初始容量（16）和默认负载因子（0.75）
    ...
    /**
     * Initializes or doubles table size（重点）.  If null, allocates in
     * accord with initial capacity target held in field threshold.
     * Otherwise, because we are using power-of-two expansion, the
     * elements from each bin must either stay at same index, or move
     * with a power of two offset in the new table.
     *
     * @return 初始化数组后的 or  扩容数组后的  Table，每次扩容2倍（尾插法）
     */
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            ...
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&//扩容2倍:oldCap << 1，左移1位，相当于oldCap乘以2的1次方
            ...
            for (int j = 0; j < oldCap; ++j) {  //如果旧的数组中有数据，循环
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null)
                        ...
                        do {
                            next = e.next;
                            if ((e.hash & oldCap) == 0) {// 如果为0，元素位置在扩容后数组中的位置没有发生改变（低位）
                                if (loTail == null)
                                    loHead = e;// 头节点
                                else
                                    loTail.next = e;
                            ...
                            else {//不为0，元素位置在扩容后数组中的位置发生了改变，新的下标位置是（原下标位置+原数组长）
                                if (hiTail == null)
                                    hiHead = e;
                                else
                                    hiTail.next = e;
    ...
    final void treeifyBin(Node<K,V>[] tab, int hash) {
        ...
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)
            ...
            TreeNode<K,V> hd = null, tl = null;//开始树化
            do {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if (p.hash == hash &&
                ...
                if (p instanceof TreeNode)
                    ...
                    } while ((e = e.next) != null);//如果e存在下一个节点，那么继续去匹配下一个节点
                }
            ...
            if (node != null && (!matchValue || (v = node.value) == value ||
                ...
                if (node instanceof TreeNode)
                    ...
                    p.next = node.next;//// 如果node节点不是首节点，此时p是node的父节点，由于要删除node，所有只需要把p的下一个节点指向到node的下一个节点即可把node从链表中删除了
     ...
     */
    private void writeObject(java.io.ObjectOutputStream s)//自定义的序列化
        throws IOException {
